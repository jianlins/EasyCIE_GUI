...
//          }
    ...
    class CpeJPanelLogger extends NLPDBLogger {
        ...
        public CpeJPanelLogger() {
            ...
            try {
                ...
                for (CpeCasProcessor cp : cpes) {
                    if (cp.getName().toLowerCase().indexOf("writer") == -1) {
                        continue;
                    }
                    NameValuePair[] paras = cp.getConfigurationParameterSettings().getParameterSettings();
                    for (NameValuePair para : paras) {
                        if (para.getName().equals(SQLWriterCasConsumer.PARAM_DB_CONFIG_FILE)) {
