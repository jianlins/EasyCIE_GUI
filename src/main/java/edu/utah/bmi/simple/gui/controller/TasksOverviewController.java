...
//
	...
	private void refreshDebugView() {
		TaskFX debugTask = mainApp.tasks.getTask("debug");
		if (debugTask != null) {
			String debugClassName = debugTask.getExecutes().get(1).getValue().getSettingValue();
			try {
				Class debugCls = Class.forName(debugClassName);
				Method m = debugCls.getMethod("showResults");
				m.invoke(null);
			} catch (ClassNotFoundException e) {
				e.printStackTrace();
			} catch (NoSuchMethodException e) {
