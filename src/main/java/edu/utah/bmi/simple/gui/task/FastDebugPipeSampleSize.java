 ...
 */
@Deprecated
public class FastDebugPipeSampleSize extends RunEasyCIE {


    protected static FastDebugPipeSampleSize fastDebugPipe = null;
    ...
    public static FastDebugPipeSampleSize getInstance(TasksFX tasks) {
        if (fastDebugPipe == null) {
            fastDebugPipe = new FastDebugPipeSampleSize(tasks);
        }
        return fastDebugPipe;
    }

    public FastDebugPipeSampleSize() {

    }

    public FastDebugPipeSampleSize(TasksFX tasks) {
        this.tasks = tasks;
        refreshPipe();
    }
...
//                        AnnotationPrinter.PARAM_INDICATION, "After DocInferenceAnnotator\n"});
        ...
        if (bunchInfRule.length() > 0) {
            runner.addAnalysisEngine(BunchMixInferenceWriter.class, new Object[]{BunchMixInferenceWriter.PARAM_BUNCH_COLUMN_NAME, "BUNCH_ID",
                    ...
                    BunchMixInferenceWriter.PARAM_VERSION, "-1"});
